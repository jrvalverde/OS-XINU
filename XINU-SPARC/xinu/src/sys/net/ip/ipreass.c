...
struct	ep	*pep;
{
...
#ifdef DEBUG
	kprintf("ipreass(0x%08x) assembling packet\n", pep);
#endif
	...
	for (i=0; i<IP_FQSIZE; ++i) {
		...
		if (piq->ipf_id != pip->ip_id)
			continue;
		if (!IP_SAMEADDR(piq->ipf_src, pip->ip_src))
			continue;
		/* found a match */
		if (ipfadd(piq, pep) == 0) {
			signal(ipfmutex);
	...
	ipfqt[firstfree].ipf_q = newq(IP_MAXNF, QF_WAIT);
	if (ipfqt[firstfree].ipf_q < 0) {
		freebuf(pep);
		signal(ipfmutex);
		return 0;
	}
	IP_COPYADDR(ipfqt[firstfree].ipf_src, pip->ip_src);
