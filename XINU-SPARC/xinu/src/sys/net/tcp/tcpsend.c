     ...
     Bool	rexmt;
{
    ...
    IP_COPYADDR(pip->ip_src, ptcb->tcb_lip);
    IP_COPYADDR(pip->ip_dst, ptcb->tcb_rip);
	...
	tcprmss(ptcb, pip);	/* set receive MSS option */
    }
...
#ifdef SJCDEBUG
    rec_tcp_seg(pep, 0, "TCPSEND");  /* 0 => send ack */
#endif
    tcph2net(ptcp);
    ptcp->tcp_cksum = 0;
    ptcp->tcp_cksum = tcpcksum(pip);
#ifdef DEBUG
    kprintf("tcpsend: sending %d bytes\n", datalen);
#endif
