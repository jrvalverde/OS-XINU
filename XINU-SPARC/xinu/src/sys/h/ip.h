...
/* IP address macros */
/* IP_SAMEADDR: are both ip addresses the same?				*/
/*	#define IP_SAMEADDR(ip1,ip2) blkequ(ip1, ip2, IP_ALEN)		*/
#define IP_SAMEADDR(ip1,ip2) (*((int *) ip1) == *((int *) ip2))
/* IP_COPYADDR: copy IP addresses (to,from)				*/
/*	#define IP_COPYADDR(ipto,ipfrom) blkcopy(ipto, ipfrom, IP_ALEN)	*/
...
#define	IP_TTL		30	/* Initial time-to-live value		*/
...
extern	int		ipproc();
#define	IPSTK		4096	/* stack size for IP process		*/
