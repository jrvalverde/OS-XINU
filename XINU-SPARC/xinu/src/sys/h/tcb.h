...
#define	TCPQLEN		15	/* TCP process port queue length	*/
...
#define	TCPE_RESET		-10
#define	TCPE_REFUSED		-11
#define	TCPE_TOOBIG		-12
#define	TCPE_TIMEDOUT		-13
#define	TCPE_URGENTMODE		-14
#define	TCPE_NORMALMODE		-15
...
#define	TCBF_NOREAD	0x200	/* users can read from the descriptor	*/
#define	TCBF_NOWRITE	0x400	/* users can write to the descriptor	*/
...
struct tcb {
	...
	int	tcb_srt;	/* smoothed Round Trip Time (1/100 sec)	*/
	int	tcb_rtde;	/* Round Trip deviation estimator (1/100)*/
	int	tcb_persist;	/* persist timeout value (1/100 sec)	*/
	int	tcb_keep;	/* keep alive timeout value (1/100 sec)	*/
	int	tcb_rexmt;	/* retransmit timeout value (1/100 sec)	*/
...
#ifdef XSELECT
 	int     tcb_writepid;   /* pid of proc selecting for writing    */
 	int     tcb_readpid;    /* pid of proc selecting for reading    */

	unsigned char tcb_nbio;	/* flag for non-blocking I/O  		*/
#endif
};
