...
#ifdef XINU8
	xcontrol(readdtable(0), TCMODER);
	xcontrol(readdtable(0), TCNOECHO);
	xcontrol(readdtable(0), TCNOINT);
	return(TRUE);
#else
 ...
 */
#ifdef XINU8	/* #include <tty.h> done above */
#define TWOSECONDS 20000		/* fairly machine dependent */
int ttwait()
{
    int i = TWOSECONDS;

    while (--i) {
	if (xcontrol(readdtable(0), TCICHARS, 0, 0) > 0) {
	    return(FALSE);
	}
    }
    return(TRUE);
