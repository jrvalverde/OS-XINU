...
#endif
#else
#define fastgetc(p)	getc(p)
#define fastputc(c, p)	putc(c, p)
#endif /* XINU8 */
...
char *frname, *toname;
{
    ...
    if (fd >= 0) {
	child = xxinu_execv("/bin/cp", 20, readdtable(fd), readdtable(fd),
			    readdtable(fd), 3, argv);
	if (child >= 0) {
	    xresume(child);
	    xreceive();
	    close(fd);
...
#define FOFFSET 47
    ...
    if(llength(lp) <= FOFFSET) return ABORT;
    (VOID) strcpy(fn, curbp->b_fname);
    cp = fn + strlen(fn);
    bcopy(&lp->l_text[FOFFSET], cp, llength(lp) - FOFFSET);
    cp[llength(lp) - FOFFSET] = '\0';
#ifdef XINU8
    return(FALSE);
#else
    return lgetc(lp, 2) == 'd';
