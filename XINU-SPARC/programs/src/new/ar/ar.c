...
#ifdef XINU8
  ascdate(member.date, timestring);
  printf (" %2d/%2d %6d %6.6s %5.5s %4.4s %s\n",
	  member.uid, member.gid,
	  member.size, timestring, timestring + 12, timestring + 7,
	  member.name);
#else
     ...
     int appendflag;
{
...
#ifdef XINU8
      int filesize = getsize(archive);

      outdesc = open (archive, O_WRONLY, 0);
      if (outdesc < 0)
	pfatal_with_name (archive);
      lseek(outdesc, filesize, L_SET);
#else
  ...
  if (symdef_flag || symdef_exists)
      ...
      else
	{
	  ...
	  this->info.date = 0;
	  this->info.size = 0;
	  this->info.uid = 60;	/* xinu uid at Purdue */
	  this->info.gid = 60;	/* xinu gid at Purdue */
	  this->info.mode = 0666;
	  this->info.symdefs = 0;
     ...
     char *outname;
{
  ...
  /* Advance member's time to current time.  */
  bzero (symdef_header.ar_date, sizeof symdef_header.ar_date);
  xgetutim(&now);
  sprintf (symdef_header.ar_date, "%ld", now);
#else
     ...
     struct mapelt *map, *after;
{
  ...
  if (access(name, R_OK))
    {
      fprintf(stderr, "ar: unable to access file %s\n", name);
      return 0;
    }
  if (old && newer_only)
    return 0;
  ...
  xgetutim(&this->info.date);
  if ((this->info.size = getsize(name)) < 0) {
      free(this);
      return(0);
  }
  this->info.uid = 60;		/* xinu uid at Purdue */
...
char	*str;
{
	...
	time = ut2ltim(time);			/* adjust for timezone	*/
	/* set year (1970-1999) */
