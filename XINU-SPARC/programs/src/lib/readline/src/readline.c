...
#else  /* XINU TTY_DRIVER */

static rl_prep_terminal ()
{
	xcbreakmode();
	readline_echoing_p = 1;
}
...
#else
	char *value = (char *)xmalloc (2 + strlen (entry));
#endif
      *value = *text;
...
#else
	      temp_name = (char *)alloca (1 + strlen (user_entry)
					  + strlen (&dirname[i]));
	      strcpy (temp_name, user_entry);
#endif
	      strcat (temp_name, &dirname[i]);
	    return_name:
	      free (dirname);
