...
#define	DISPCONF	"../h/dispconf.h"	/* name of dispatcher .h*/
#define	IODISP		"../h/iodisp.h"		/* name of dispat loc .h*/
	...
	char	*argv[];
{
	...
	if ( (dispconf=fopen(DISPCONF,"w") ) == NULL) {
		fprintf(stderr, "Can't write on %s\n",DISPCONF);
		exit(1);
	}
	if ( (iodisp=fopen(IODISP,"w") ) == NULL) {
		fprintf(stderr, "Can't write on %s\n",IODISP);
		exit(1);
	}
	fprintf(confh,
	...
	fprintf(dispconf,
		"/* dispconf.h (GENERATED FILE; DO NOT EDIT) */\n");
	fprintf(iodisp,
		"/* iodisp.h (GENERATED FILE; DO NOT EDIT) */\n");
	fprintf(confc, "\n#include %s\n", CONFHREF);
	...
	for (i=0; i<ndevs; i++) {
		fprintf(dispconf,"\t.globl\t_devveci%-1d\n",i);
		fprintf(dispconf,"\t.globl\t_devveco%-1d\n",i);
		fprintf(dispconf,"\t.align\t2\n");
		fprintf(dispconf,"_devveci%-1d:\n",i);
		fprintf(dispconf,"\tmtpr\t$DISABLE, $IPL\n");
		fprintf(dispconf,"\tbsbw\tioint\n");
		fprintf(dispconf,"\t.align\t2\n");
		fprintf(dispconf,"_devveco%-1d:\n",i);
		fprintf(dispconf,"\tmtpr\t$DISABLE, $IPL\n");
		fprintf(dispconf,"\tbsbw\tioint\n\n",i);
		fprintf(iodisp, "extern\tint\tdevveci%-1d();\n",i);
		fprintf(iodisp, "extern\tint\tdevveco%-1d();\n",i);
	}
