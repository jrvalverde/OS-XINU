/****************************************/
/*					*/
/*	modified to have static return	*/
/*	area malloc'ed instead		*/
/*	(note:  free is not done to	*/
/*	this returned area, but as	*/
/*	someone has said, we are	*/
/*	dealing with virtual memory)	*/
/*					*/
/*	Also, the global var char *tbuf	*/
/*	was converted to being an	*/
/*	argument in those routines	*/
/*	using the variable		*/
/*					*/
/****************************************/
char *malloc();
	...
	char *name;
{
	...
	register struct	disktab *dp =
			(struct disktab *)malloc(sizeof(struct disktab));
	...
	strcpy(cp, name);
	cp += strlen(name) + 1;
	dp->d_type = dgetstr("ty", &cp, tbuf);
	dp->d_secsize = dgetnum("se",tbuf);
	if (dp->d_secsize < 0)
		dp->d_secsize = 512;
	dp->d_ntracks = dgetnum("nt",tbuf);
	dp->d_nsectors = dgetnum("ns",tbuf);
	dp->d_ncylinders = dgetnum("nc",tbuf);
	dp->d_rpm = dgetnum("rm",tbuf);
	if (dp->d_rpm < 0)
		dp->d_rpm = 3600;
	...
	for (p = 'a'; p < 'i'; p++) {
		...
		pp->p_size = dgetnum(psize,tbuf);
		pp->p_bsize = dgetnum(pbsize,tbuf);
		pp->p_fsize = dgetnum(pfsize,tbuf);
	}
	return (dp);
}
