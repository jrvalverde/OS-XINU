...
#define	comerr(msg) {expbuf[0] = 0; numbra = 0; ssignal(sem_sys_gen); return(msg); }

    swait(sem_sys_gen);
	if (sp == 0 || *sp == '\0') {
		if (*ep == 0)
			{ssignal(sem_sys_gen); return("No previous regular expression");}
	...
	register char	*p1;
{
	...
	if (circf)
		{ssignal(sem_sys_gen); return((advance(p1, p2)));}
	/*
