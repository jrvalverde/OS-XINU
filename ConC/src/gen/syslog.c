	...
	char *fmt;
{
	...
	if (pri <= 0 || PRIFAC(pri) >= LOG_NFACILITIES || (PRIMASK(pri) & LogMask) == 0)
		{ssignal(sem_sys_gen); return;}
	if (LogFile < 0)
		SYS_openlog(LogTag, LogStat | LOG_NDELAY, 0);
	...
	*b = '\0';
	...
	pid = vfork();  /* YOU KNOW THIS WON'T WORK IN ConcurrenC, DON'T YOU?	*/
			/* vfork() IS NOT ALLOWED, SO DON'T BE SURPRISED.	*/
			/* YOU WILL JUST RETURN IN THE NEXT if.			*/
	if (pid == -1)
		{ssignal(sem_sys_gen); return;}
	if (pid == 0) {
		int fd;
...
SYS_openlog(ident, logstat, logfac)
	char *ident;
	int logstat, logfac;
{
	if (ident != NULL)
