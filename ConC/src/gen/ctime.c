 ...
 *    (small place where dmsize[1]	*
 *    might change from 28 to 29	*
 *    and back)				*
 *					*
...
/* cbuf is now a local malloc'd area because it is returned	*/
/* (was destroyed on each succesive call).			*/
...
time_t *tim;
{
	...
	long hms, day;
	register int *tp;
    struct tm *pxtime = (struct tm *)malloc(sizeof(struct tm));
...
struct tm *t;
{
	...
	register int *tp;
    char *cbuf = malloc(sizeof(char) * 26);
