	...
	char *name;
{
	...
	char buf[50];
	extern char *index();
    struct ns_addr *paddr = (struct ns_addr *)malloc(sizeof(struct ns_addr));
	...
	*paddr = zero_addr;
	...
	if (hostname)
		*hostname++ = 0;
	Field(buf, paddr->x_net.c_net, 4);
	if (hostname == 0)
		return (*paddr);  /* No separator means net only */

	socketname = index(hostname, separator);
	if (socketname) {
		*socketname++ = 0;
		Field(socketname, &paddr->x_port, 2);
	}

	Field(hostname, paddr->x_host.c_host, 6);
