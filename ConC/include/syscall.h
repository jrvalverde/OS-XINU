...
/*
 *  Basically <syscall.h> with added stuff for ConcurrenC.
 */
...
/* structure constants defined */

#define SN_VALS		5		/* # vals or lens to send */
...
struct sys_hdr {
	...
	int	sys_val_len[SN_VALS];		/* value or length array*/
};

#define MALLOC(type,len)\
			(((len) > 0)	  ? (type)malloc(len) : (type)0)
#define FREE(type,buf)\
			((buf == (type)0) ? 0		      : free(buf))

int	Read_From_Pipe();

#define SYS_flush_io_pending	-99	/* flush any I/O pending for task */
#define SYS_null	0		/* no syscall			  */

/* syscall command constants (/usr/include/syscall.h) */
